...
def add_guest(list_of_guests):
    list_of_guests.append(simpledialog.askstring(None, "What is the name of the guest you want to add to the guest list?"))
...
def remove_guest(list_of_guests):
    list_of_guests.remove(simpledialog.askstring(None, "What is the name of the guest you want to remove from the list?"))
    return list_of_guests
...
def print_guests(list_of_guests):
    ...
    if len(list_of_guests) == 0:
        ...
        for i in range(len(list_of_guests)):
            s += "Guest " + str(i + 1) + ". " + list_of_guests[i] + "\n"
        messagebox.showinfo(None, s)
